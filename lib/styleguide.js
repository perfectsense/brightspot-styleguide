...
let Styleguide = function (gulp, settings = { }) {
  ...
  parseString(fs.readFileSync(path.join(this.config['project-path'], this.config['maven-pom'])), { async: false }, (err, pomXml) => {
    ...
    this.config['project-build-path'] = path.join(process.cwd(), 'target', 'gulp')
  ...
  this.path = {
    base: () => '.',
    src: (args) => {
      let normalizePath = (file) => {
        return path.join(this.config['project-path'], this.config['project-src-path'], file || '')
      }

      if (typeof args === 'string' && args.length > 0) {
        return normalizePath(args)
      }

      if (Array.isArray(args) && args.length !== 0) {
        return args.map(item => normalizePath(item))
      }

      throw new Error(`Calling styleguide.path.src() with the glob '${args}' is invalid. It should be a string or array of strings.`)
    },
  ...
  this.gulp = {
    src: (glob, options) => {
      return this._gulp.src(this.path.src(glob), options || { base: this.path.base() })
    },
    dest: (glob, options) => {
      return this._gulp.dest(path.join(this.path.build(), glob), options)
    },
    task: (name, deps, fn) => this._gulp.task(name, deps, fn),
    watch: (glob, opts, tasks) => this._gulp.watch(glob, opts, tasks)
  }
