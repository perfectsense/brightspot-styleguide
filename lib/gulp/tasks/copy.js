...
module.exports = {
  registerModule: (styleguide) => {
    ...
    styleguide._gulp.task('bsp-styleguide:copy:templates', ['bsp-styleguide:copy:createPackageFile'], () => {
      return styleguide.gulp.src(path.join(styleguide.config['project-src-path'], `**/*.${extensionPattern}`), { base: '.' })
        .pipe(styleguide.gulp.dest(''))
    })
    ...
    styleguide._gulp.task('bsp-styleguide:copy:createPackageFile', ['bsp-styleguide:copy:node_modules'], () => {
      ...
      folders.map((folder) => {
        return styleguide._gulp.src(path.join(styleguide.path.build(), '..', 'gulp/node_modules', folder, '/*'))
                .pipe(file('package.json', '{}'))
                .pipe(styleguide._gulp.dest(path.join(styleguide.path.build(), '..', 'gulp/node_modules', folder, '/')))
      })
    })
