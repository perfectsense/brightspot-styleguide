        ...
        library.forEachStyleguideGroup(function (group, groupPath) {
            ...
            function directoryTree (dirPath) {
                ...
                if (name.slice(0, 1) !== '_') {
                    ...
                    var item = {
                        name: label(name),
                        url: path.join(library.urlPrefix, urlPath, originalUrlSearch)
                    }

                    if(fs.statSync(dirPath).isFile()) {
                        item.name = label(name);
                        item.url = path.join(library.urlPrefix, urlPath.slice(0, -config['json-suffix'].length), originalUrlSearch);
                        ...
                        try {
                            ...
                            item.children = fs.readdirSync(dirPath)
                                .map(child => directoryTree(path.join(dirPath, child)))
                                .filter(e => !!e);
                        } catch(ex) {
                            if(ex.code == "EACCES")
                                return null;
            ...
            examples.push(directoryTree(groupPath));
