...
var Styleguide = function(gulp, settings = { }) {
    ...
    this.path = {
        src: (glob) => {
            return path.join(this.config['project-path'], this.config['project-src-path'], glob || '')
        },

        build: (glob) => {
            return glob || this.config['project-build-path']
        }
    }
}
