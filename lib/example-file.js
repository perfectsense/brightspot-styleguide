...
module.exports = function (config, req, res, context) {
    ...
    handlebars.registerHelper('replace', function (name, options) {
        ...
        data[name] = new handlebars.SafeString(options.fn(data[name]));
    });

    // Marks the template within as being overrideable.
    handlebars.registerHelper('overridable', function (options) {
        var data = options.data.root;
        var override = data._override;

        if (override) {
            return new handlebars.SafeString(override(data));

        } else {
            return new handlebars.SafeString(options.fn(data));
        }
    });
