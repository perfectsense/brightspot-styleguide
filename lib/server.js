...
module.exports = function (config) {
    ...
    app.use(function (req, res, next) {
        ...
        library.forEachStyleguideGroup(function (group, groupPath) {
            var examples = [ ];
            var secondaryExamples = [ ];
            var previousPath = "";
            var findGroups = function (groupPath, group) {
                ...
                fs.readdirSync(groupPath).forEach(function (example) {
                    ...
                    if (example.slice(0, 1) !== '_') {
                        ...
                        if (fs.statSync(examplePath).isDirectory()) {
                            previousPath = library.urlPrefix + '/' + group + '/' + example + originalUrlSearch;
                            lastExample = {
                                name: label(example),
                            ...
                            secondaryExamples = [];
                            findGroups(examplePath, group);
                        } else if(/\.(json)$/i.test(examplePath) && group) {

                            secondaryExamples.push({
                                name: label(example),
                                url: previousPath+ '/' + example + originalUrlSearch
                            });
                        }

                        if ( lastExample !== null && secondaryExamples.length > 0) {
                            lastExample.children = secondaryExamples;
                        }
        ...
        if (require('./render-content')(config, req, res, context, examplePath, '/main.hbs')) {
            return;
        }
