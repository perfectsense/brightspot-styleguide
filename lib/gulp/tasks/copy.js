    ...
    function config () {
      return filter((file) => {
        ...
        if (`source` in config) {
          ...
          // determine where each template should be moved and update the manifest data
          ...
          // move files.
          if (manifestData.overrides.length > 0) {
            manifestData.overrides.forEach((file) => {
          ...
          fs.writeFileSync(path.join(styleguide.path.build(), 'styleguide-manifest.json'), JSON.stringify(manifestData, null, `\t`))
        }

        // throw if any errors were detected
        if (manifestData.errors.length > 0) {
          throw new Error(manifestData.errors.map((error) => {
            return `\n${error.message}\n`
          }))
        }
      })
