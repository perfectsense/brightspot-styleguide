...
function resolveTemplatePath (data, requestPath, project) {
  traverse(data).forEach(function (value) {
    ...
    if (template) {
      ...
      // path is absolute, exists and therefore needs no further processing
...
function resolveDataUrlPaths (data, currentDataPath, project) {
  ...
  traverse(data).forEach(function (value) {
    ...
    if (dataUrl) {
      ...
      /*
      * was my parent dataUrl'd?
      * if it was, then my paths need to be updated relative to that parent path
      */
      } else if (this.parent && this.parent.node._dataUrl) {
        dataPath = path.resolve(this.parent.node._absolutePath, dataPath)
        absolutePath = path.dirname(dataPath)
      // treat it as a relative path to the current contextPath
      } else {
        dataPath = path.resolve(path.dirname(currentDataPath), dataPath)
        absolutePath = path.dirname(dataPath)
      }
...
module.exports = function (config, req, res, context) {
  ...
  handlebars.registerHelper('defineBlock', function (name, options) {
    ...
    if (extend) {
      ...
      let contextPath = options.data.root._contextPath || options.data.root._template
      ...
      absolutePath = path.resolve(contextPath, extend)
