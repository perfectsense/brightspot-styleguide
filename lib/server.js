  ...
  function addTemplatePath(basePath) {
    ...
    if (fs.existsSync(mavenPomPath)) {
      ...
      function add(templatePath) {
        ...
        var renderPath = path.join(templatePath, 'render');

        if (fs.existsSync(renderPath)) {
          templatePaths.push(renderPath);
        }
      }
    ...
    if (filePath) {
      ...
      function renderTemplate(context) {
        ...
        if (!compiledTemplate) {
          ...
          var prependedPaths = templatePaths.map(function (templatePath) {
            return path.join(templatePath, namePath + '.hbs');
          });
          ...
          if (/\{\{\s*>/.test(template)) {
            ...
            if (!partialsRegistered) {
              templatePaths.forEach(function (templatePath) {
                rrs(templatePath).forEach(function (filePath) {
      ...
      if(req.query.ajax) {
        template = handlebars.compile(fs.readFileSync(findInStyleguide('/ajax.hbs'), 'utf8'));
      }
      else {
        template = handlebars.compile(fs.readFileSync(findInStyleguide('/iframe.hbs'), 'utf8'));
      }
