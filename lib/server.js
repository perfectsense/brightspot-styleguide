...
module.exports = function (config) {
  ...
  app.use('/styleguide/_', express.static(path.join(__dirname, '..', 'node_modules')));
          ...
          fs.readdirSync(groupPath).forEach(function (example) {
            if (example.slice(0, 1) !== '_') {
              ...
              if (fs.statSync(examplePath).isDirectory()) {
                examples.push({
                  name: label(example),
                  url: '/' + examplePath + originalUrlSearch
                });
              }
            }
          });
  ...
  app.listen(config.port, config.host, function () {
    logger.success('Started on http://' + config.host + ':' + config.port + '/styleguide/');
  });
};
