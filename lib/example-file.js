  ...
  traverse(data).forEach(function (value) {
    ...
    if (templatePath) {
      ...
      if (!path.isAbsolute(templatePath) &&
        ...
        // Absolute paths (/styleguide/foo/bar) are resolved by going backwards up through the directory tree until
        // a package.json is found.
        let context = this.node._dataUrl || path.join(project._projectPath, path.dirname(templatePath))
        basePath = Util.closestParentWithFile(context, 'package.json')
      } else {
        // relative path
      ...
      if (!value._template) {
        throw new Error(templatePath + " template doesn't exist!")
      }
    }
  })
}
...
module.exports = function (config, req, res, context) {
  ...
  handlebars.registerHelper('defineBlock', function (name, options) {
    ...
    if (extend) {
      ...
      } else if (path.isAbsolute(extend)) {
        // Absolute paths are resolved by the closest parent directory with a "package.json"
        contextPath = Util.closestParentWithFile(path.join(project._projectPath, path.dirname(extend)), 'package.json')
      } else {
