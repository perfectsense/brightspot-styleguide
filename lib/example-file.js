...
module.exports = function (config, req, res, context) {
    ...
    var wrapperPath = projectLibrary.findStyleguideFile('_wrapper.json');

    // I'm putting this in here. if there is no '_wrapper' specified in the data, we skip this. This allows
    // example files to NOT have a wrapper if we do not want it
    if (wrapperPath && data._wrapper) {
