    ...
    traverse(data).forEach(function (value) {
        ...
        if (dataUrl) {
            ...
            if (dataUrl.slice(0, 1) === '/') {
                ...
                if (!dataPath) {
                    throw new Error("Can't find " + dataUrl + " in " + projectLibrary.name + " styleguide!");
                }
...
module.exports = function (config, req, res, context) {
    ...
    handlebars.registerHelper('jsonObject', function (context, options) {
        ...
        for (var key in jsonObjectData) {
            if(count > 0) {
                jsonObject += ',';
            }

            jsonObject += key + '=' + jsonObjectData[key];
            count++;
        }

        jsonObject += '}';

        return new handlebars.SafeString(jsonObject);
    });
