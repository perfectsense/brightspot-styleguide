...
function resolveTemplatePath (data, requestPath, project) {
  traverse(data).forEach(function (value) {
    ...
    if (template) {
      ...
      if (templatePath.split(path.sep)[1] === 'node_modules') {
    ...
    if (dataUrl) {
      ...
      try {
        ...
        throw err
            ...
            // update the template path relative to the parent template that _dataUrl'd it
      data._template = path.join(path.dirname(dataUrl), data._template)
...
module.exports = function (config, req, res, context) {
  ...
  function compile (name, relPath) {
    ...
    return handlebars.compile(fs.readFileSync(selectedProject.findFile(name + extension, relPath), 'utf8'))
  }
  ...
  handlebars.registerHelper('defineBlock', function (name, options) {
    ...
    if (extend) {
      var template = compile(extend, path.dirname(options.data.root._template))
