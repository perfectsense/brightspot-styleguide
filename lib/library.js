...
Library.prototype.mergeConfig = function (config) {
    ...
    if (cfg){
        logger.success('Configured with: '+ path.join(this._styleguidePath, '_config.settings'));
        return _.extend({ }, config, cfg);
    }
    logger.success('Configured with default "_config.settings" file');
...
Library.prototype.getConfig = function () {
    try {
        return JSON.parse(fs.readFileSync(path.join(this._styleguidePath, '_config.settings'), "utf8"));
    } catch (ex) {
        return null;
    }
};
