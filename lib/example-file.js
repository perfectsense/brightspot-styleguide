...
module.exports = function (config, req, res, context) {
    ...
    if (data._wrapper !== false) {
        var effectiveWrapperLibrary = data._view ? selectedLibrary : projectLibrary;
        var wrapperPath = effectiveWrapperLibrary.findStyleguideFile(data._wrapper ? data._wrapper : '_wrapper.json');
