...
module.exports = function (config) {
    ...
    app.use(function (req, res, next) {
        ...
        rrs(config.webappPaths[0]).forEach(function (filePath) {
            if (path.extname(filePath) !== '.vars' || path.basename(filePath).indexOf('_') === 0) {
                return;
            }

            var name = path.basename(filePath, path.extname(filePath));
            var category = path.dirname(filePath).match(/[^\/]+$/).toString();

            if(!designExamples[label(category)]) {
                designExamples[label(category)] = [];
            }

            designExamples[label(category)].push({
                name: label(name),
                url: '/design/' + name
            });
        ...
        // Design pages?
        if (require('./design')(config, req, res, context)) {
            return;
        }
