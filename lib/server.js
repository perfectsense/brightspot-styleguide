...
module.exports = function (config) {
    ...
    function addLibrary(basePath) {
        ...
        if (library.isLibrary()) {
            library.urlPrefix = '/' + library.name.replace(/ /, '-').toLowerCase();
    ...
    config = _.extend({ }, config, projectLibrary.getConfig());
    ...
    app.use(function (req, res, next) {
        ...
        // Which theme to display?
        if (config.themes){
            context.resetThemeUrl = url.parse(req.originalUrl, true);
            var availableThemes = context.availableThemes = [ ];
            config.themes.forEach(function (theme, i) {
                var key = 't' + i;

                delete context.resetThemeUrl.query[key];

                var themeUrl = url.parse(req.originalUrl, true);
                var themeQuery = themeUrl.query;
                var availableTheme = {
                    theme: theme
                };

                availableTheme.url = url.format(themeUrl);
                availableThemes.push(availableTheme);
                console.log(availableTheme);
            });

            delete context.resetThemeUrl.search;
            context.resetThemeUrl = url.format(context.resetThemeUrl);
        }
        ...
        fs.readdirSync(examplePath).forEach(function (file) {
            if (file.slice(0, 1) !== '_' && path.extname(file) === config['json-suffix']) {
                ...
                traverse(JSON.parse(fs.readFileSync(exampleFilePath, 'utf8'))).forEach(function (value) {
                    if (this.key === 'options' &&
