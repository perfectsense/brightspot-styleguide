...
module.exports = function (config, req, res, context) {
        ...
        if (dataUrl) {
            ...
            if (dataUrl.slice(0, 1) === '/') {
                _.each(context.availableLibraries, function (al) {
                    var newDataPath = al.library.findStyleguideFile(dataPath);
    ...
    var template = handlebars.compile(fs.readFileSync(context.library.findStyleguideFile(req.query.ajax ? '/ajax.hbs' : '/iframe.hbs'), 'utf8'));
