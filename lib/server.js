...
module.exports = function (config) {
    ...
    addLibrary(config['project-path']);
    addChildLibraries(path.join(config['project-path'], 'node_modules'));
    ...
    app.use(function (req, res, next) {
        ...
        // Finds all LESS variable files.
        var variablesGroups = { };

        library.forEachWebappFile(function (filePath) {
            if (path.extname(filePath) !== '.vars') {
                return;
            }

            var exampleName = path.basename(filePath, path.extname(filePath));
            var groupName = path.basename(path.resolve(filePath, '..'));
            var group = variablesGroups[groupName];

            if (!group) {
                group = variablesGroups[groupName] = {
                    name: 'Variables: ' + label(groupName),
                    examples: [ ]
                };
            ...
            group.examples.push({
                name: label(exampleName),
                url: library.urlPrefix + '/variables/' + exampleName
            });
        });
