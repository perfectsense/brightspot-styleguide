...
module.exports = function (config) {
    ...
    if (config.middleware) {
      config.middleware.forEach(function(middleware) {
        var middlewarePath = path.resolve(config['project-path'] + '/' + middleware.path + '.js');
        var middlewarePathExec;
        if (fs.existsSync(middlewarePath)) {
          middlewarePathExec = path.dirname(middlewarePath) + '/' + path.basename(middlewarePath, '.js');
          app.use(middleware.urlPattern, require(middlewarePathExec));
        } else {
          console.warn('WARNING: middleware not found at', middlewarePath);
        }
      });
    }
