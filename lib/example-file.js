...
function resolveDataUrlPaths(data, currentDataPath, library) {
    var levelLibraryMapStack = [];

    // The reason for using map instead of forEach is that in-place root element update doesn't work.
    return traverse(data).map(function (value) {
        var dataUrl = value._dataUrl;

        if (dataUrl) {
            var dataUrlParts = dataUrl.split(':');
            var effectiveLibrary = null;
            var effectiveUrl = null;

            // namespaced data url
            if (dataUrlParts.length == 2) {

                if (!dataUrlParts[1].startsWith('/')) {
                    throw new Error("Namespaced data url should contain absolute path!");
                }

                var libraryNamespace = dataUrlParts[0];

                if (!libraryNamespace) {
                    throw new Error("Library name is empty!");
                }

                library._config.libraries.some(function (styleguideLibrary) {

                    if (styleguideLibrary.namespace == libraryNamespace) {
                        effectiveLibrary = styleguideLibrary;
                        effectiveUrl = dataUrlParts[1];
                        return true;
                    }
                })

                if (!effectiveLibrary) {
                    throw new Error("Can't find " + libraryNamespace + "!");
                }

                levelLibraryMapStack.push({'level': this.level, 'library': effectiveLibrary});

            } else {
                var levelLibraryMap = levelLibraryMapStack.pop();

                if (levelLibraryMap) {

                    if (levelLibraryMap.level < this.level) {
                        effectiveLibrary = levelLibraryMap.library;
                        levelLibraryMapStack.push(levelLibraryMap);
                    }
                }

                effectiveUrl = dataUrl;

                if (effectiveLibrary == null && effectiveUrl.startsWith('/')) {
                    effectiveLibrary = library;
                }
            }

            var dataPath = path.join.apply(path, effectiveUrl.split('/'));

            if (effectiveLibrary != null) {
                dataPath = effectiveLibrary.findStyleguideFile(dataPath);

                if (!dataPath) {
                    throw new Error("Can't find " + effectiveUrl + " in " + effectiveLibrary.name + " styleguide!");
                }
                ...
                if (!dataPath) {
                    throw new Error("Can't find " + effectiveUrl + " relative to " + currentDataPath + "!");
                }
            }
...
module.exports = function (config, req, res, context) {
    ...
    data = resolveDataUrlPaths(data, filePath, selectedLibrary);
    ...
    if (data._wrapper !== false && !data._view) {
        ...
        while (wrapperPath) {
            ...
            wrapper = resolveDataUrlPaths(wrapper, wrapperPath, projectLibrary);
