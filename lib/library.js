    ...
    if (webappPath) {
        ...
        if (targetPath) {
            rrs(webappPath).forEach(function (filePath) {
                var targetFilePath = path.join(targetPath, path.relative(webappPath, filePath));

                if (fs.existsSync(targetFilePath)) {
                    callback(targetFilePath, filePath);
...
Library.prototype.findVariableFiles = function () {
    ...
    this.forEachWebappFile(function (filePath, name) {
        if (path.extname(name) === '.vars') {
            varFiles.push({
                path: filePath,
                name: name.slice(0, -5).split(fs.sep).join('/')
            });
        }
    });
