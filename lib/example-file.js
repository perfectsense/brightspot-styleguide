...
module.exports = function (config, req, res, context) {
    ...
    // Replaces a property with the template within.
    handlebars.registerHelper('replace', function (name, options) {
        var value = this[name];

        this[name] = new handlebars.SafeString(options[options.hash['no-with'] || value ? 'fn' : 'inverse'](value));
    });

    // Marks the template within as being overrideable.
    handlebars.registerHelper('overridable', function (options) {
        var overrideTemplate = this._overrideTemplate;

        if (overrideTemplate) {
            return new handlebars.SafeString(overrideTemplate(this));
    ...
    handlebars.registerHelper('override', function (name, options) {
        ...
        this._overrideTemplate = options.fn;
